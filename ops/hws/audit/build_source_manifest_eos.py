...
OUTPUT_PATH = ROOT / "data" / "audit" / "source" / "eos.json"
...
def build_row(series_payload: dict[str, Any], product: dict[str, Any], wave_file: str) -> dict[str, Any]:
    ...
    power = pick_spec_value(spec_groups, ["Мощность", "Номинальная потребляемая мощность", "Power"])
    if not power:
        power = normalize_text(product.get("eos_power_kw"))
...
def main() -> None:
    ...
    parser.add_argument(
        "--input-glob",
        default="hws-eos-wave-*-detailed.json",
    ...
    print(f"brand=eos total={manifest['total']} output={output_path}")

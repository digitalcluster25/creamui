...
OUTPUT_PATH = ROOT / "data" / "audit" / "source" / "easysteam.json"
...
def build_row(series_payload: dict[str, Any], product: dict[str, Any], wave_file: str) -> dict[str, Any]:
    ...
    power = pick_spec_value(spec_groups, ["Мощность", "Power"])
    ...
    if option_groups:
        ...
        for group in option_groups:
            ...
            if values:
                expected_variation_count *= len(values)
    ...
    return {
        ...
        "source_has_image": bool(main_image),
        "has_image": bool(main_image),
        "main_image": main_image,
        "source_has_short_description": bool(short_description),
        "has_short_description": bool(short_description),
        "short_description": short_description,
        "source_has_full_description": bool(full_description),
        "has_full_description": bool(full_description),
        "full_description": full_description,
        "source_has_specs": bool(spec_groups),
        "has_specs": bool(spec_groups),
        "source_has_variants": bool(option_groups),
...
def build_manifest_from_payloads(paths: list[Path]) -> dict[str, Any]:
    ...
    return {
        "brand": {"name": "EasySteam", "slug": "easysteam"},
...
def main() -> None:
    ...
    parser.add_argument(
        "--input-glob",
        default="hws-easysteam-wave-*-detailed.json",
    ...
    print(f"brand=easysteam total={manifest['total']} output={output_path}")
